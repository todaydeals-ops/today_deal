...
const a1 = {
  ...
  main: [
    ...
    H2("왜 하필 0.3㎛가 기준일까"),
    P0("필터 성능을 0.3㎛에서 재는 데는 이유가 있어요. 큰 먼지는 관성으로 부딪혀 잘 걸리고, 아주 작은 입자(0.1㎛ 이하)는 공기 중에서 마구 움직이다(확산) 필터에 붙어 잘 걸리는데요. 하필 " + B("0.3㎛ 부근이 두 효과가 모두 약한 ‘사각지대’") + "라 가장 안 잡혀요. 그래서 이 크기를 99.95% 잡는 필터(H13)는, 그보다 크거나 작은 초미세먼지는 같거나 더 잘 거른다고 보면 됩니다."),
    P("필터 교체 주기도 광고와 현실이 갈려요. ‘1년 사용’ 표기가 많지만, 미국 EPA는 경부하 기준에서도 보통 60~90일을 권하고, " + B("반려동물·흡연·고농도 환경에선 더 짧아지는") + " 편이에요. 프리필터(먼지망)는 2~4주 세척으로 오래 쓰지만, 헤파·탈취는 매년 갈아야 하는 소모품이라는 점을 잊지 마세요."),
    P("마지막으로 인증과 오존이에요. 한국은 " + B("CA인증(한국공기청정협회)") + "이 청정성능·탈취·소음·오존 네 가지를 함께 검증해요. 특히 일부 방식은 부산물로 오존이 나올 수 있는데, 인증 기준은 오존 발생량을 0.03ppm 이하로 제한하니 이 표시를 확인하는 게 안전합니다."),
    P("용량은 넉넉한 쪽이 이득이에요. 방보다 한 단계 큰 모델을 고르면 같은 청정도를 " + B("더 낮은 풍량으로 = 더 조용하고 전기도 덜 쓰며") + " 얻거든요. ‘딱 맞는 용량’보다 ‘약간 큰 용량’이 실사용에선 더 편한 편입니다."),
...
const a2 = {
  ...
  main: [
    ...
    H2("광고가 안 말하는 현실 세 가지"),
    P0("첫째, " + B("애초에 못 넣는 음식") + "이 있어요. 닭뼈·생선뼈·조개껍데기는 처리 대상이 아니라 일반쓰레기고요. 진짜 함정은 국물·기름·수분이에요. 이게 많으면 처리 시간이 확 늘고 악취·잔여물이 생겨, 미생물·건조형 모두 애를 먹는 편입니다."),
    P("둘째, " + B("고장과 A/S") + "예요. 실사용 불만에서 자주 나오는 고장은 히터·건조 불량, 악취, 분쇄·모터 문제 순인데요. A/S에 보통 1주 안팎, 부품 대기는 더 걸리기도 해요. 특히 신생 브랜드는 폐업 시 부품 수급이 막힐 수 있으니, 본체 가격만큼 A/S 지속성도 봐야 하는 수준이에요."),
    P("셋째, 미생물식은 ‘넣기만 하면 되는 기계’가 아니에요. " + B("수분·유분이 과하면 균이 약해져") + " 1~2주 제 기능을 못 하기도 하고, 1~2개월마다 굳은 잔여물을 손으로 걷어내야 하고요. 조용하고 친환경이라는 장점의 이면에 이런 ‘손이 가는’ 부분이 있다는 걸 감안하세요."),
    P("탈취도 방식마다 약점이 달라요. " + B("미생물식은 탈취가 가장 취약") + "해서 관리가 부실하면 ‘상한 우유’ 같은 냄새가 날 수 있고, 건조·분쇄식은 활성탄 필터가 수명을 다하면 냄새가 확 올라오는 편이에요. 본체값만큼 ‘소모품을 제때 갈 수 있는가’가 냄새를 좌우합니다."),
    H2("사기 전 체크리스트"),
    NUMLIST(INDIGO, [
...
const a3 = {
  ...
  main: [
    ...
    H2("인치 말고 — 패널·주사율·외장 모니터"),
    P0("같은 인치라도 " + B("패널이 IPS냐 OLED냐") + "가 작업을 가르는데요. OLED는 검정·색감·응답이 좋아 영상·HDR에 강하지만, 코딩·문서처럼 화면 틀이 종일 고정된 작업엔 번인 위험이 있어요. 반대로 IPS는 번인이 사실상 없고 밝기 유지가 좋아, 정적인 UI를 오래 보는 작업에 무난한 편입니다."),
    P("주사율도 용도로 갈려요. 60→120Hz는 스크롤·커서에서 대부분 체감하지만, " + B("문서·웹 같은 정지 화면에선 차이가 사라지고") + " 배터리만 더 먹어요. 고주사율이 의미 있으려면 게이밍처럼 실제로 그 프레임을 그려줄 GPU가 받쳐줘야 하고요."),
    P("그리고 데스크 작업이 잦다면 " + B("‘작은 휴대형 + 외장 모니터’ 조합") + "이 현실적인 답이에요. 인치를 키워 무게를 떠안기보다, 가벼운 걸 들고 다니다 자리에선 큰 화면에 꽂는 거죠. (참고로 블루라이트 차단 안경은 눈 피로 감소 근거가 약해요 — 20분마다 먼 곳 보기·화면 거리·밝기 관리가 핵심입니다.)"),
    P("배터리도 오해가 있어요. ‘용량(Wh)이 크면 오래 간다’ 같지만, 실제 지속시간은 " + B("용량보다 CPU·디스플레이 효율") + "이 더 좌우해요. 57Wh 울트라북이 90Wh 게이밍보다 오래 가기도 하거든요. ‘몇 Wh’ 숫자보다, 같은 무게에서 효율 좋은 조합인지를 보세요."),
    H2("사기 전 체크리스트"),
    NUMLIST(ORANGE, [
...
const a4 = {
  ...
  main: [
    ...
    H2("흡입력이 떨어졌다면 — 진짜 원인"),
    P0("‘새것 같던 흡입력이 약해졌다’면 십중팔구 " + B("필터 막힘") + "이 1순위예요. 그다음이 먼지통 과포화·흡입 덕트에 뭉친 보풀, 메인브러시 엉킴, 그리고 연결부 고무 씰이 틀어진 누설이고요. ‘막힌 데 없는데 약하다’면 씰을 의심해 보세요. 대부분 새로 사야 할 문제가 아니라 청소·교체로 돌아옵니다."),
    P("내비게이션 방식도 우리 집 환경에 맞춰 봐야 해요. " + B("라이다(LDS)는 빛을 직접 쏘아 어두운 집·검은 바닥에도 강하고") + ", 카메라(vSLAM)는 천장·가구 특징을 읽어 저렴하지만 어두우면 성능이 떨어져 조명이 필요해요. 자이로식은 지그재그로 훑고 시간이 갈수록 오차가 쌓이는 편이고요."),
    P("마지막은 엉킴·접점 관리예요. 메인브러시는 " + B("양 끝 베어링 안쪽") + "이 가장 잘 막히고, 앞 보조바퀴(캐스터)는 통째로 빼서 머리카락 덩어리를 걷어내야 해요. 충전이 안 될 땐 로봇 하단 금속 단자와 충전독 핀을 마른 천으로 닦아주면 대부분 해결됩니다."),
    P("자동비움 도크도 함정이 있어요. 손이 편해지는 대신 " + B("전용 먼지봉투를 약 8주마다 교체") + "해야 해 유지비가 붙고요. 또 검은 카펫·짙은 바닥은 낙하방지 센서가 ‘헛절벽’으로 오인해 그 위를 안 닦거나 멈추기도 하니, 우리 집 바닥 색도 한 번 고려해보세요."),
    H2("사기 전 체크리스트"),
    NUMLIST(OLIVE, [
