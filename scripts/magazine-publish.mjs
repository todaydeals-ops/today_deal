...
const a = {
  ...
  main: [
    ...
    P("장마철 빨래건조에도 유용해요. ‘빨래건조 모드’는 목표 습도를 최저로 두고 강풍을 돌리는데, " + B("선풍기·서큘레이터를 함께 틀어 바람을 분산") + "시키면 반나절 걸릴 빨래도 두세 시간으로 줄어드는 편이에요(빨래가 많으면 대용량 컴프레서가 유리). 단 떨어지는 물이 배출구로 들어가지 않게 거리를 두세요."),
    H2("사기 전 체크리스트"),
    NUMLIST(INDIGO, [
